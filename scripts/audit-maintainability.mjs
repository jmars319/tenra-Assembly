...
function appRoutes() {
  const appRoot = path.join(root, ...(config.appRouteRoot ?? "apps/webapp/src/app").split("/"));
  return walk(appRoot).filter((file) => /\/(page|route)\.tsx?$/.test(file)).map((file) =>
    relative(file).replace(new RegExp(`^${(config.appRouteRoot ?? "apps/webapp/src/app").replaceAll("/", "\\/")}\\/`), "").replace(/\/(page|route)\.tsx?$/, "").replace(/^(page|route)\.tsx?$/, "/")
  ).sort();
}
...
function desktopCommands() {
  const tauriRoot = path.join(root, "apps", "desktopapp", "src-tauri", "src");
  if (!fs.existsSync(tauriRoot)) return [];
  return walk(tauriRoot)
    .filter((file) => file.endsWith(".rs"))
    .flatMap((file) => [...fs.readFileSync(file, "utf8").matchAll(/fn\s+([a-zA-Z0-9_]+)\s*\(/g)].map((match) => match[1]))
    .sort();
}
...
if (expectedContracts) {
  const actualContracts = { packageNames: packageNames(), packageExports: packageExports(), appRoutes: appRoutes(), desktopMenuLabels: desktopMenuLabels(), desktopCommands: desktopCommands() };
